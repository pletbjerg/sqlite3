  ...
  outputs = { self, nixpkgs }:
    let
      ...
      sqlite3-help = pkgs.writeShellScriptBin "sqlite3-help" ''
        ...
        exec "$browser" "${sqlite3-doc + "/share/doc/index.html"}"
      '';

      # Convenient derivation to amalgamate the above derivations into a
      # single derivation.
      sqlite3 = pkgs.symlinkJoin {
        name = "sqlite3";
        paths =
          [
            sqlite3-bin
    ...
    in
    {
      packages.${system} = {
        default = sqlite3;
        inherit sqlite3-bin sqlite3-doc sqlite3-help;
      };

      # For details on `mkShell`, see
